...
export default function KanjiDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-4">
      <Link href="/kanji" className="text-sm text-violet-400 hover:underline mb-6 block">← Back</Link>

      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 mb-6 text-center">
        <div className="text-8xl mb-4 text-zinc-100">{kanji.character}</div>
        {jisho && (
          <>
            {jisho.japanese?.[0]?.reading && (
              <div className="text-lg text-zinc-400 mb-2">{jisho.japanese[0].reading}</div>
            )}
            {jisho.senses && (
